...
5- Managed OS disks of the removed VMs
6- Unmanaged OS disks' VHD & status blobs of the removed VMs (Optional via setting parameter "remove_unmanaged_osdisk_vhd_blob" to "yes")
7- All managed data disks attached to the removed VMs
...
Workflow  VMResourcesSaverAutomation {
    Param(
        ...
        [string]$remove_unmanaged_osdisk_vhd_blob = 'no',
    ...
    if ($VMList) {
        ...
        foreach -parallel -ThrottleLimit $Throttle_Limit ($virtualmachine in $VMList) {
            try {
                ...
                if ($virtualmachine["OSDiskVHD"] -and ($remove_unmanaged_osdisk_vhd_blob -ieq "yes")) {
